...
class CloudWatchResponse(BaseResponse):
    ...
    def describe_alarm_history(self):
        ...
        template = self.response_template(DESCRIBE_ALARM_HISTORY_TEMPLATE)
        return template.render(alarms=alarms)
...
DESCRIBE_ALARM_HISTORY_TEMPLATE = """<DescribeAlarmHistoryResponse xmlns="http://monitoring.amazonaws.com/doc/2010-08-01/">
<DescribeAlarmHistoryResult>
   <AlarmHistoryItems>
      {% for a in alarms %}
      <member>
        <Timestamp>{{ a['Timestamp'] }}</Timestamp>
        <HistoryItemType>{{ a['HistoryItemType'] }}</HistoryItemType>
        <AlarmName>{{ a['AlarmName'] }}</AlarmName>
        <HistoryData>{{ a['HistoryData'] }}</HistoryData>
        <HistorySummary>{{ a['HistorySummary'] }}</HistorySummary>
      </member>
      {% endfor %}
   </AlarmHistoryItems>
  </DescribeAlarmHistoryResult>
